...
import { Check, UserCheck, AlertCircle, Trash2 } from 'lucide-react';
...
export default function CheckIn({ t, role, syncTick }) {
  ...
  useEffect(() => {
    if (selectedTeam && selectedLeg) {
      ...
      const found = players.find(p => p.player_key === key && p.status !== 'Substituted');
  ...
  const handleSave = (e) => {
    ...
    setMsg(t('yes') === 'نعم' ? 'تم حفظ حالة التحضير!' : 'Check-in recorded!');
    loadData();
  ...
  const handleDeleteCheckin = (key) => {
    ...
    if (window.confirm(t('delete_confirm'))) {
      deleteCheckIn(key);
      setMsg(t('yes') === 'نعم' ? 'تم إلغاء تحضير اللاعب بنجاح!' : 'Check-in cleared successfully!');
      loadData();
      setTimeout(() => setMsg(''), 3000);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="grid-2">
        ...
        <form onSubmit={handleSave} className="card">
          ...
          <div style={{ background: 'rgba(14, 19, 43, 0.3)', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
            <p style={{ fontSize: '0.9rem', color: '#9ca3af', marginBottom: '1rem' }}>{t('checkin_desc')}</p>
          ...
          {/* Matched Player Card */}
          {matchedPlayer ? (
            <div className="card" style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'var(--border-color)', margin: '1rem 0' }}>
              <div className="flex-between">
                <div>
                  <h3 style={{ fontSize: '1.1rem', color: '#fff', marginBottom: '0.25rem' }}>{matchedPlayer.full_name}</h3>
                  <span className={`badge ${matchedPlayer.player_type.toLowerCase()}`}>
                    {matchedPlayer.player_type === 'Starter' ? t('starter') : t('reserve')}
                  </span>
                  <span className="number-text ml-2 mr-2" style={{ color: '#9ca3af', fontSize: '0.9rem' }}>({matchedPlayer.player_key})</span>
                </div>
                <div>
                  {matchedPlayer.medical_notes !== 'None' && (
                    <span className="badge refunded">🩺 {t(matchedPlayer.medical_notes) || matchedPlayer.medical_notes}</span>
                  )}
                </div>
              </div>
              ...
              <div style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginTop: '0.5rem' }}>
                  <label className="flex-gap-2" style={{ cursor: 'pointer' }}>
                    ...
                    />
                    <span className="form-label" style={{ color: '#fff' }}>{t('jersey')}</span>
                  </label>
                  ...
                  <label className="flex-gap-2" style={{ cursor: 'pointer' }}>
                    ...
                    />
                    <span className="form-label" style={{ color: '#fff' }}>{t('warmup')}</span>
                  </label>
                  ...
                  <label className="flex-gap-2" style={{ cursor: 'pointer' }}>
                    ...
                    />
                    <span className="form-label" style={{ color: '#fff' }}>{t('cleared')}</span>
                  </label>
                </div>
              </div>
            ...
            <div style={{ padding: '2rem', textAlign: 'center', background: 'rgba(239, 68, 68, 0.05)', borderRadius: '8px', color: '#f87171', border: '1px dashed rgba(239,68,68,0.2)', margin: '1rem 0' }}>
              <AlertCircle size={32} style={{ margin: '0 auto 0.5rem auto', display: 'block' }} />
              {t('yes') === 'نعم' 
                ? `لا يوجد لاعب مسجل في رمز الخانة ${selectedTeam}-${selectedLeg}`
                : `No active player occupies slot ${selectedTeam}-${selectedLeg}`}
            </div>
          )}
        </form>
        ...
        <div className="card">
          <div className="card-header">
            <h2 className="card-title">⚠️ {t('not_checked_title')} ({unchecked.length})</h2>
          </div>
          {unchecked.length === 0 ? (
            <div style={{ color: '#34d399', textAlign: 'center', padding: '2rem', fontWeight: 600 }}>
              {t('no_unchecked')}
            </div>
          ) : (
            <div style={{ maxHeight: '420px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {unchecked.map(player => (
                ...
                >
                  <div>
                    <span className="number-text" style={{ fontWeight: 'bold', color: 'var(--color-primary-hover)', marginInlineEnd: '0.5rem' }}>
                      {player.player_key}
                    </span>
                    <strong style={{ color: '#fff' }}>{player.full_name}</strong>
                  </div>
                  <span className={`badge ${player.player_type.toLowerCase()}`}>
                    {player.player_type === 'Starter' ? t('starter') : t('reserve')}
                  </span>
                </div>
              ))}
            </div>
      ...
      <div className="card mt-4">
        <div className="card-header">
          <h2 className="card-title">✅ {t('yes') === 'نعم' ? 'قائمة الحضور والجاهزية المعتمدة' : 'Confirmed Check-ins'} ({checkIns.length})</h2>
        </div>
              ...
              {checkIns.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: '2rem' }}>
                    {t('yes') === 'نعم' ? 'لا يوجد لاعبون محضرون حالياً.' : 'No checked-in players yet.'}
                  </td>
                </tr>
              ) : (
                checkIns.map(c => {
                  const pName = players.find(x => x.player_key === c.player_key && x.status !== 'Substituted')?.full_name || t('unregistered');
                  return (
                    <tr key={c.player_key}>
                      <td className="number-text" style={{ fontWeight: 'bold' }}>{c.player_key}</td>
                      ...
                      <td>
                        <span className={`badge ${c.jersey_received ? 'approved' : 'refunded'}`}>
                          {c.jersey_received ? t('yes') : t('no')}
                        </span>
                      </td>
                      <td>
                        <span className={`badge ${c.warmup_completed ? 'approved' : 'refunded'}`}>
                          {c.warmup_completed ? t('yes') : t('no')}
                        </span>
                      </td>
                      <td>
                        <span className={`badge ${c.cleared_to_compete ? 'approved' : 'refunded'}`}>
