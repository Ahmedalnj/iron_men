...
function timeStringToSeconds(val) {
  ...
  // Replace Arabic colons or separator if any
  val = val.replace('٫', '.');
...
export default function Timing({ t, role, syncTick }) {
  ...
  useEffect(() => {
    ...
    const rawSumSec = (leg1 || 0) + (leg2 || 0) + (leg3 || 0) + (leg4 || 0) + (leg5 || 0) + (leg6 || 0);

    const barrierPen = (timing.barriers_knocked || 0) * settings.barrier_knock_penalty_sec;
    const skipPen = (timing.obstacles_skipped || 0) * settings.obstacle_skip_penalty_sec;
    const helpPen = (timing.outside_help_count || 0) * settings.outside_help_penalty_sec;
  ...
  const handleSave = (e) => {
    ...
    // Build saved entry with parsed seconds
    const savedEntry = {
      ...timing,
    ...
    upsertTimingEntry(savedEntry);
    setMsg(t('yes') === 'نعم' ? 'تم تسجيل وتحديث أوقات الفريق بنجاح!' : 'Timing details updated successfully!');
    
    // Reload updated entry
    const entry = getTimingEntryByTeam(selectedTeam);
    setTiming(entry);
  ...
  const handleDeleteTiming = () => {
    ...
    if (window.confirm(t('delete_confirm'))) {
      deleteTimingEntry(selectedTeam);
      ...
      setMsg(t('yes') === 'نعم' ? 'تم مسح نتيجة الفريق بنجاح!' : 'Team timing cleared successfully!');
      setTimeout(() => setMsg(''), 3000);
    }
  };
  ...
  const getPlayerName = (suffix) => {
    ...
    const p = players.find(x => x.player_key === key && x.status !== 'Substituted');
    ...
    <div>
        ...
        <div className="alert-banner critical">
          ...
          <div>
            ...
            <div className="alert-banner-desc">
              {t('yes') === 'نعم' ? 'يجب تسجيل الدخول كميقاتي أو مسؤول لتسجيل الأوقات.' : 'You must switch your role to Timekeeper or Admin in the header to enter race timing.'}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid-2">
        ...
        <form onSubmit={handleSave} className="card">
          ...
          {/* Leg Times */}
          <h3 className="mb-2" style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-primary-hover)' }}>
            🏃‍♂️ {t('leg_times')}
          </h3>
            ...
            {[1, 2, 3, 4, 5, 6].map(num => (
              <div className="form-group" key={num}>
                <label className="form-label" style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#9ca3af', maxWidth: '120px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
                    {getPlayerName(num)}
                  </span>
                </label>
                <input 
                  ...
                  disabled={!canEdit}
                  onChange={(e) => setRawLegTimes({ ...rawLegTimes, [`leg${num}`]: e.target.value })}
                />
              </div>
            ))}
          </div>
          ...
          {/* Core Penalties */}
          <h3 className="mt-4 mb-2" style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-primary-hover)' }}>
            ⚠️ الجزاءات الجماعية / Team-Level Penalties
          </h3>
          <div className="form-grid">
            <div className="form-group">
              <label className="form-label">{t('barriers')} (+{settings.barrier_knock_penalty_sec}s)</label>
              <input 
                type="number" 
                className="form-input timer-text"
                min={0}
                ...
                onChange={(e) => handleInputChange('barriers_knocked', e.target.value)}
              />
            </div>

            <div className="form-group">
              <label className="form-label">{t('obstacles')} (+{settings.obstacle_skip_penalty_sec}s)</label>
              <input 
                type="number" 
                className="form-input timer-text"
                min={0}
                ...
                onChange={(e) => handleInputChange('obstacles_skipped', e.target.value)}
              />
            </div>

            <div className="form-group">
              <label className="form-label">{t('outside_help')} (+{settings.outside_help_penalty_sec}s)</label>
              <input 
                type="number" 
                className="form-input timer-text"
                min={0}
                ...
                onChange={(e) => handleInputChange('outside_help_count', e.target.value)}
              />
            </div>
          </div>
            ...
            {[1, 2, 3, 4, 5, 6].map(num => (
              <div className="form-group" key={num}>
                ...
                <input 
                  ...
                  onChange={(e) => handleInputChange(`player${num}_penalty_sec`, e.target.value)}
                />
              </div>
            ))}
          </div>
          ...
          <div className="form-grid">
            <div className="form-group">
              ...
              <select 
                ...
                disabled={!canEdit}
                onChange={(e) => handleInputChange('result_status', e.target.value)}
              >
                <option value="Pending">{t('Pending')}</option>
                <option value="Finished">{t('yes') === 'نعم' ? 'منتهي Finished' : 'Finished'}</option>
            ...
            <div className="form-group">
              ...
              >
                ...
                <option value="الحكم أحمد">الحكم أحمد</option>
                <option value="الحكم خالد">الحكم خالد</option>
                <option value="الحكم سالم">الحكم سالم</option>
                <option value="الحكم علي">الحكم علي</option>
                <option value="حكم آخر">حكم آخر</option>
              </select>
            </div>
          </div>
          ...
          {canEdit && (
            <div className="flex-gap-2 mt-4">
              ...
              <button type="button" className="btn btn-danger" onClick={handleDeleteTiming}>
                {t('delete')}
              </button>
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="card" style={{ borderColor: 'var(--color-primary-glow)', boxShadow: '0 0 15px var(--border-glow)' }}>
            <div className="card-header" style={{ borderColor: 'rgba(225, 29, 72, 0.2)' }}>
              <h2 className="card-title" style={{ color: 'var(--color-primary-hover)' }}>📊 {t('live_computed')}</h2>
            </div>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div className="flex-between">
                <span className="form-label">{t('raw_time')}</span>
                <span className="timer-text" style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
                  {secondsToTimeString(liveTotals.rawSumSec) || '--:--'}
                  <span style={{ fontSize: '0.8rem', color: '#9ca3af', marginInlineStart: '0.5rem' }}>
              ...
              <div className="flex-between">
                ...
                <span className="timer-text text-danger" style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
              ...
              <div className="flex-between" style={{ padding: '0.5rem 0' }}>
                <span className="form-label" style={{ fontSize: '1rem', fontWeight: 800, color: '#fff' }}>
                  {t('final_time')}
                </span>
                <div>
                  {['DNS', 'DNF', 'Injured', 'Disqualified', 'Withdrawn'].includes(timing.result_status) ? (
                    <div style={{ textAlign: 'end' }}>
                      <span className="badge refunded" style={{ fontSize: '1rem', padding: '0.4rem 0.8rem' }}>
                        {timing.result_status} ({settings.dnf_dns_injury_value}s)
                      </span>
                      <span className="timer-text block mt-1" style={{ display: 'block', fontSize: '0.85rem', color: '#6b7280' }}>
                        Literal time: {secondsToTimeString(liveTotals.totalResultSec)}
                      </span>
                    </div>
                  ) : (
                    <span className="timer-text" style={{ fontSize: '2.2rem', fontWeight: 800, color: 'var(--color-gold)', textShadow: '0 0 10px var(--color-gold-glow)' }}>
                      {secondsToTimeString(liveTotals.totalResultSec) || '--:--'}
                      <span style={{ fontSize: '1rem', color: '#fff', marginInlineStart: '0.5rem' }}>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {[1, 2, 3, 4, 5, 6].map(num => (
                <div key={num} className="flex-between" style={{ padding: '0.4rem 0.75rem', background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-color)', borderRadius: '6px' }}>
                  <span className="number-text" style={{ fontWeight: 'bold', color: 'var(--color-primary-hover)' }}>{selectedTeam}-{num}</span>
