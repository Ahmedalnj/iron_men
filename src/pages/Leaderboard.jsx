...
export default function Leaderboard({ t, syncTick }) {
  ...
  const top3 = leaderboard
    .filter((team) => !team.is_incomplete && team.has_times)
    .slice(0, 3);

  // Format local clock
  const formattedClock = currentTime.toLocaleTimeString("ar-LY", {
    hour: "2-digit",
    minute: "2-digit",
    second: "2-digit",
  });
      ...
      <div className="projector-container">
        ...
        <div className="projector-header">
          <div className="projector-title">
            🏆{" "}
            {t("yes") === "نعم"
              ? "لوحة الترتيب المباشرة - البطل الحديدي 2026"
              : "Live Standings - Iron Champion 2026"}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "2rem" }}>
            <div
              className="projector-time timer-text"
              style={{
              ...
              >
                ...
                <div
                  style={{
                    height: "50px",
                    width: "100%",
                    background: "linear-gradient(to top, #451a03, #78350f)",
                    borderTopLeftRadius: "8px",
                    borderTopRightRadius: "8px",
                ...
                leaderboard.map((team, index) => {
                  ...
                  return (
                    <tr key={team.team_number} className={rowClass}>
                      <td>
                        <div
                          className={`projector-rank-badge ${team.rank === 1 ? "gold" : team.rank === 2 ? "silver" : team.rank === 3 ? "bronze" : "other"}`}
                          style={{ color: team.rank <= 3 ? "#000" : "#fff" }}
                        >
                          {team.rank}
                        </div>
                      </td>
                      ...
                      <td>
                        {team.is_incomplete ? (
                          ...
                          <span
                            className="timer-text projector-timer"
                            style={{
                              color:
                      ...
                      <td>
                        <span
                          className={`badge ${team.result_status.toLowerCase()}`}
                          style={{
                            fontSize: "1.2rem",
            ...
            {leaderboard.length === 0 ? (
              ...
              leaderboard.map((team) => (
                <tr
                  key={team.team_number}
                  className={
                    !team.is_incomplete && team.has_times
                      ? `rank-${team.rank}`
                      : ""
                  }
                >
                  <td>
                    <div
                      className={`rank-badge ${!team.is_incomplete && team.has_times ? (team.rank === 1 ? "gold" : team.rank === 2 ? "silver" : team.rank === 3 ? "bronze" : "other") : "other"}`}
                    >
                      {team.rank}
                    </div>
                  </td>
