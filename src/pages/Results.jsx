...
export default function Results({ t, syncTick }) {
  ...
  useEffect(() => {
    const loadResults = () => {
      ...
      data.sort((a, b) => {
        if (a.ranking_time_seconds === null && b.ranking_time_seconds === null) return 0;
  ...
  const filtered = results.filter(row => {
    const matchesSearch = row.team_name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    ...
    <div>
      ...
      <div className="card" style={{ padding: '1rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', minWidth: '200px' }}>
          ...
          >
            <option value="ALL">{t('yes') === 'نعم' ? 'كل النتائج' : 'All Standings'}</option>
            <option value="FINISHED">{t('yes') === 'نعم' ? 'منتهي فقط' : 'Finished Only'}</option>
            <option value="INCOMPLETE">{t('yes') === 'نعم' ? 'غير مكتمل (DNF/DNS)' : 'Incomplete (DNF/DNS)'}</option>
            <option value="PENDING">{t('yes') === 'نعم' ? 'قيد الانتظار' : 'Pending Only'}</option>
          </select>
        </div>
      </div>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} style={{ textAlign: 'center', padding: '2rem' }}>
                  {t('yes') === 'نعم' ? 'لا توجد نتائج مطابقة لبحثك.' : 'No matching results found.'}
                </td>
              </tr>
            ) : (
              filtered.map((row, index) => {
                ...
                return (
                  <tr key={row.team_number} className={rankClass}>
                    <td>
                      {rankNum ? (
                        <div className={`rank-badge ${rankNum === 1 ? 'gold' : rankNum === 2 ? 'silver' : rankNum === 3 ? 'bronze' : 'other'}`}>
                          {rankNum}
                        </div>
                      ) : (
                    ...
                    <td className="number-text" style={{ fontWeight: 800 }}>{row.team_number}</td>
      ...
      <div className="card">
        <h3 className="card-title mb-2">⚖️ ملاحظات التحكيم والميقاتي / Officiating Notes</h3>
        <p style={{ fontSize: '0.85rem', color: '#9ca3af', lineHeight: '1.5' }}>
          * {t('yes') === 'نعم' ? 'الفرق التي تسجل حالات DNS (لم يبدأ) أو DNF (لم ينهِ) أو إصابة يتم استبعادها من الترتيب التنافسي المباشر وتثبيتها بقيمة زمنية جزائية قدرها 9999 ثانية لدفعها لآخر الجدول.' : 'Teams recording DNF, DNS, or Injured status are automatically pushed to the bottom of the scoreboard using the 9999s penalty sentinel value to preserve the competitive ranking structure.'}
        </p>
      </div>
    </div>
  );
