...
import { getTeams, getPlayers, upsertPlayer, deletePlayer, getSettings } from '../state';
  ...
  return (
    <div>
        ...
        <form onSubmit={handleSave} className="card">
          <div className="card-header">
            <h2 className="card-title">
              {editingPlayer ? `✏️ ${t('edit_player')}` : `➕ ${t('add_player')}`}
            </h2>
          </div>
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              >
                <option value="None">{t('yes') === 'نعم' ? 'لا يوجد' : 'None'}</option>
                <option value="Asthma">{t('yes') === 'نعم' ? 'ربو' : 'Asthma'}</option>
                <option value="Diabetes">{t('yes') === 'نعم' ? 'سكري' : 'Diabetes'}</option>
                <option value="Heart">{t('yes') === 'نعم' ? 'أمراض قلب' : 'Heart condition'}</option>
                <option value="Other">{t('yes') === 'نعم' ? 'أخرى' : 'Other'}</option>
              </select>
            </div>
          </div>
      ...
      <div className="table-container">
        <div className="card-header" style={{ borderBottom: 'none', padding: '1rem' }}>
          <h3 className="card-title">👥 Roster: {selectedTeam} ({starters.length} Starters, {reserves.length} Reserves)</h3>
        </div>
            ...
            {filteredPlayers.length === 0 ? (
              <tr>
                <td colSpan={canEdit ? 6 : 5} style={{ textAlign: 'center', padding: '2rem' }}>
                  {t('yes') === 'نعم' ? 'لا يوجد لاعبون مسجلون لهذا الفريق.' : 'No players registered on this team yet.'}
                </td>
              </tr>
            ) : (
              filteredPlayers.map(player => (
                <tr key={player.id} className={player.status === 'Substituted' ? 'opacity-50' : ''}>
                  <td className="number-text" style={{ fontWeight: 'bold' }}>{player.player_key}</td>
                  ...
                  <td>
                    {player.medical_notes === 'None' 
                      ...
                      : <span className="badge refunded">{t(player.medical_notes) || player.medical_notes}</span>}
                  </td>
                  {canEdit && (
                    <td>
                      <div className="flex-gap-2">
